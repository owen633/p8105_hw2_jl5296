...
```
*   The number of unique locations included in the dataset is `r nrow(uniq_loc_data)`. The answer to whether every state has been represented is `r (nrow(uniq_state_data) >= 50)` (There are actually `r nrow(uniq_state_data)` states being observed counting DC as one). The state being mostly observed is `r Mode(tidy_brfss_data$state)`.
